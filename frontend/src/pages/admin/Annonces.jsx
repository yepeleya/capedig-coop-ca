...
const CATEGORIE_LABEL = {
  cooperative: 'Coopérative', marche: 'Marché', technique: 'Technique',
  formation: 'Formation', logistique: 'Logistique',
}
...
function AnnonceModal({ initial, onClose, onSave }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (statut === 'programmee' && !datePublication) {
      setErreur('La date de publication est requise pour une annonce programmée.')
      return
    }
    ...
    formData.append('image_actuelle', imageRetiree ? '' : (initial?.image || ''))
          ...
          <div>
            ...
            {imagePreview ? (
              <div className="relative w-full h-36 rounded-xl overflow-hidden border border-gray-200 mb-2">
                ...
                <button type="button" onClick={handleRetirerImage}
            ...
            <p className="text-[11.5px] text-gray-400 mt-1">
              Certaines annonces n'ont pas besoin d'image — ce champ est optionnel.
            </p>
          </div>
...
export default function Annonces() {
  ...
  const charger = () => {
    setLoading(true)
    api.get('annonces/index.php', { admin: 1 })
      .then(data => setAnnonces(Array.isArray(data) ? data : []))
      .catch(() => setMsgError('Impossible de charger les annonces.'))
  ...
  const filtered = annonces.filter(a => categorie === 'Toutes' || a.categorie === categorie)

  const handleSave = async (formData, isUpdate) => {
    await api.postForm(isUpdate ? 'annonces/update.php' : 'annonces/create.php', formData)
    flash(setMsgSuccess, isUpdate ? 'Annonce mise à jour avec succès.' : 'Annonce créée avec succès.')
    setModal(null)
    charger()
  }
  ...
  const toggleStatut = async (a) => {
    ...
    try {
      await api.post('annonces/update_statut.php', { id: a.id, statut: nouveau })
      charger()
    } catch (e) {
      flash(setMsgError, e.message)
    }
  ...
  const publiees = annonces.filter(a => a.statut === 'publiee').length
  const programmees = annonces.filter(a => a.statut === 'programmee').length
      ...
      <div className="ml-[240px]">
        ...
        <main className="p-8">
          <div className="flex items-center justify-between mb-6"
            ...
            <div>
              <h1 className="font-display text-[26px] font-bold text-gray-900">Annonces</h1>
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="border-b border-gray-100 text-[12px] font-bold
                  ...
                  <th className="px-5 py-3.5">Annonce</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {loading && (
                  <tr><td colSpan={6} className="px-5 py-6 text-center text-gray-400 text-[13.5px]">Chargement...</td></tr>
                )}
                {!loading && filtered.length === 0 && (
                  <tr><td colSpan={6} className="px-5 py-6 text-center text-gray-400 text-[13.5px]">Aucune annonce.</td></tr>
                )}
                {!loading && filtered.map(a => {
                  const isPub = a.statut === 'publiee'
                  const isProg = a.statut === 'programmee'
                  return (
                      ...
                      <td className="px-5 py-3.5">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-[11.5px] text-gray-400">{CATEGORIE_LABEL[a.categorie] || a.categorie}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-5 py-3.5 text-[13.5px] text-gray-600">{a.auteur || '—'}</td>
                      <td className="px-5 py-3.5">
                        <button onClick={() => !isProg && toggleStatut(a)} disabled={isProg}
                          className="flex items-center gap-2 text-[12.5px] font-semibold">
                          <span className={`w-2 h-2 rounded-full ${isPub ? 'bg-capedig-vert' : isProg ? 'bg-blue-500' : 'bg-capedig-orange'}`} />
                          <span className={isPub ? 'text-capedig-vert' : isProg ? 'text-blue-500' : 'text-capedig-orange'}>
