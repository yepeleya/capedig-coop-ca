...
export default function ActualiteDetail() {
  ...
  useEffect(() => {
    fetch(`/api/actualites/index.php?id=${id}`)
      .then(r => r.ok ? r.json() : [])
      .then(data => {
        const item = Array.isArray(data) ? data[0] : null
        if (item) setActu(item)
        else setErreur(true)
      })
              ...
              {img && (
                <div className="bg-black">
                  {video ? (
                    ...
                    <img src={img} alt={actu.titre} className="w-full max-h-[420px] object-cover" />
                  )}
                </div>
              )}
              <div className="p-8">
                <div className="flex items-center gap-3 mb-4">
                  <span className="text-[10.5px] font-bold tracking-[1.4px] px-3 py-1 rounded
                                   bg-capedig-orange text-white uppercase">
                    {actu.categorie || 'Actualité'}
                  </span>
                  <span className="text-gray-400 text-[12.5px]">{actu.date}</span>
                </div>
                ...
                <div
                  ...
                  dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(actu.contenu) }}
                />
              </div>
            </article>
          )}
